            ...
            for (int dy = row; dy <= radius; dy++)
            {
                ...
                for (int dx = Mathf.CeilToInt(nextStartSlope * (float)dy); dx >= 0; dx--)
                {
                    ...
                    if (dx * dx + dy * dy < radiusSquare)
                    {
                        ...
                        if (Data.Tile.Type.Wall == tile.type || (startSlope >= centerSlope && centerSlope >= endSlope))
                        {
                            this.tiles.Add(tile);
                        }
                    }
                    ...
                    else if (Data.Tile.Type.Wall == tile.type)            // 블록킹 섹션 시작. 새로운 종료 기울기를 설정하고 다음 행 또는 열에서 재귀적 스캔 시작
                    {
                        blocked = true;
                        nextStartSlope = rightSlope;
